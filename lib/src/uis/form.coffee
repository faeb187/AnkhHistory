...
module.exports = (->
      ...
      switch type
        when 'select'
          ...
          for o in fldO
            ...
            $inp.appendChild $opt
          $fld.appendChild $inp
          if lab    then $fld.appendChild $$ '<label/>', for: objId, 'data-lang': lab
    ...
    listen: ( $ui ) ->
      ...
      if $range.length
        $$.listen $range, 'input', ( e ) ->
          e.target.previousSibling.value = cf.format e.target.value, code: 'CHF'
      
        $$.listen $range.previousSibling, 'keyup', ( e ) ->
          e.target.nextSibling.value = e.target.value
      
      if $inps.length then $$.listen $inps, 'change', ( e ) ->
        $elm  = e.target
  ...
  return {
    ...
    init: ( opt ) ->
      ...
      # MAN id, ids, target
      if !id or !ids or !$t then return
      
      # UI markup
      $ui           = ui.$tpl.cloneNode()
      $ui.id        = id
      $ui.className = if type then 'ui-' + type else 'ui-form'
