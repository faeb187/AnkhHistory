...
gulp.task('helpers', function() {
  return gulp.src('src/helpers/*.coffee').pipe(coffee({
    bare: true
...
gulp.task('bundle', function() {
  ...
  return gulp.src('*.js', {
    read: false
  }).pipe(shell(['browserify app.js ' + '-r ./helpers/dom ' + '-r ./helpers/obs ' + '-r ./helpers/site ' + '-r ./helpers/stalker ' + '-r ./helpers/state ' + '-r ./conf/back ' + '-r ./conf/cnt ' + '-r ./conf/copyright ' + '-r ./conf/footer ' + '-r ./conf/front ' + '-r ./conf/lang ' + '-r ./conf/main ' + '-r ./conf/nav ' + '-r ./conf/slider ' + '-o ./app.min.js']));
});

gulp.task('default', gulp.series("uis", "helpers", "pug", "styl", "conf", "designs", "sites", "coffee", "bundle"), (done) => {
  return done();
});
